...
function parseDeliveryIniFile($configPath = null, $configFile = null, $sections = true)
{
    ...
    if (!empty($conf)) {
        ...
        echo "Adserver could not read the default configuration file for the {$pluginType} plugin";
        exit(1);
    }
    ...
    if (file_exists(MAX_PATH . '/var/INSTALLED')) {
        echo "Adserver has been installed, but no configuration file was found.\n";
        exit(1);
    }
    // Adserver hasn't been installed, so delivery engine can't run
    echo "Adserver has not been installed yet -- please read the INSTALL.txt file.\n";
    exit(1);
}
