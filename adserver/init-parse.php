 ...
 *                           Default is Adserver's /var directory.
 * @param string $configFile The configuration file name (eg. "geotargeting").
 *                           Default is no name (ie. the main Adserver
 *                           configuration file).
...
function parseIniFile($configPath = null, $configFile = null, $sections = true, $type = '.php')
{
    ...
    // Get the host name that Adserver is being accessed via, so that
    ...
    if ($installing)
    {
        ...
        if (!$configFile)
        {
            // Adserver hasn't been installed, so use the distribution
    ...
    // Check to ensure Adserver hasn't been installed
    ...
    // Adserver hasn't been installed, so use the distribution .ini file
    return @parse_ini_file(MAX_PATH . '/etc/dist.conf.php', $sections);
}
